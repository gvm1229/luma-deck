...
function updateConnector(node: HTMLElement, element: SceneElement): void {
  ...
  stick.style.display = drawing.stickVisible ? 'block' : 'none'
...
export function getConnectorDrawing(rawWidth: number, rawHeight: number, rawProgress: unknown) {
  ...
  const strokeWidth = Math.min(6, height * .2)
  const headBase = width - arrowWidth
  const headGap = 12
  const stickEnd = headBase - headGap - strokeWidth / 2
  const progress = Math.min(Math.max(Number(rawProgress ?? 1), 0), 1)
  const headVisible = progress === 1
  const stickVisible = stickEnd > 0
  return { width, height, strokeWidth, headBase, headGap, stickX2: stickVisible ? stickEnd * progress : 0, stickVisible, headVisible }
}
